  ...
  const fetchSensorData = async () => {
    try {
      ...
      if (data.feeds && data.feeds.length > 0) {
        ...
        setSensorData({
          waterLevel: parseFloat(feed.field1) || 0,
          emergencySwitch: feed.field2 === "1" ? "ON" : "OFF",
          pumpState: feed.field3 === "1" ? "ON" : "OFF",
          timestamp: feed.created_at,
        });
      }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex items-center justify-between p-6 bg-secondary/50 rounded-xl border border-border">
                  <div className="flex items-center gap-3">
                    <Power className={cn(
                      "h-8 w-8",
                      sensorData?.pumpState === "ON" ? "text-success" : "text-destructive"
                    )} />
                    <div>
                      <p className="text-sm text-muted-foreground">Pump State</p>
                      <p className={cn(
                  ...
                  <div className={cn(
                    "w-16 h-8 rounded-full transition-all duration-300",
                    sensorData?.pumpState === "ON" ? "bg-success" : "bg-destructive"
                  )}>
                    <div className={cn(
                      "w-8 h-8 rounded-full bg-white shadow-lg transition-transform duration-300",
                      sensorData?.pumpState === "ON" && "translate-x-8"
                ...
                <div className="flex items-center justify-between p-6 bg-secondary/50 rounded-xl border border-border">
                  <div className="flex items-center gap-3">
                    <AlertTriangle className={cn(
                      "h-8 w-8",
                      sensorData?.emergencySwitch === "ON" ? "text-success" : "text-destructive"
                    )} />
                    <div>
                      <p className="text-sm text-muted-foreground">Emergency Switch</p>
                      <p className={cn(
                        ...
                        {loading ? "Loading..." : sensorData?.emergencySwitch || "OFF"}
                      </p>
                    </div>
                  </div>
                  <div className={cn(
                    "w-16 h-8 rounded-full transition-all duration-300",
                    sensorData?.emergencySwitch === "ON" ? "bg-success" : "bg-destructive"
                  )}>
                    <div className={cn(
                      "w-8 h-8 rounded-full bg-white shadow-lg transition-transform duration-300",
                      sensorData?.emergencySwitch === "ON" && "translate-x-8"
                    )}></div>
                  </div>
                </div>
              </div>
